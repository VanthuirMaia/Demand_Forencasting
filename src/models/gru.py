...
def construir_gru(
    ...
    """Constrói e compila um GRU para regressão de séries temporais."""
    modelo = keras.Sequential()
    modelo.add(layers.Input(shape=input_shape))

    for i, n_unidades in enumerate(unidades):
        return_sequences = i < len(unidades) - 1
        modelo.add(layers.GRU(n_unidades, return_sequences=return_sequences))
...
def treinar_gru(
    ...
    """Treina o GRU com EarlyStopping sobre val_loss."""
